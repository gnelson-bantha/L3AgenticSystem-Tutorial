...
- **Dot-fade / binary-fade decorative section (opt-in).** The default stylesheet ships
  ...
  swap the `dot-fade*` class names for their `binary-fade*` equivalents. The full-bleed
  background sits flush against an adjacent `hr.section-divider` via adjacent-sibling
  rules (`.dot-fade + hr.section-divider` and `hr.section-divider:has(+ .dot-fade)`), so
  the divider must be the section's **immediate** sibling. On architecture pages the
  `.modal` divs are emitted right after `</section>`, between the section and the next
  divider — when applying `dot-fade`/`binary-fade` to such a section, move the trailing
  `hr.section-divider` to immediately follow `</section>` (before the modals), or the
  flush rule fails silently and a gap appears.
